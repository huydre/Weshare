...
export const MessageDetail = ({ targetID, targetInfo }: { targetID: number, targetInfo: Target }) => {
    ...
    <div className="w-full bg-white flex flex-col h-full justify-between dark:bg-neutral-900">
      <div className="h-full flex flex-col ">
        <div className="flex space-x-2 items-center border-b-2 dark:border-neutral-700 py-4 px-4">
          <Avatar src={`${process.env.NEXT_PUBLIC_API_URL}media/${targetInfo.image_url}`}/>
          ...
          {messages?.map((message) =>
            message.sender === userLogged!.username ? (
              ...
              <div className="flex space-x-2 items-center pt-2 max-w-[700px]">
                <Avatar src={`${process.env.NEXT_PUBLIC_API_URL}media/${targetInfo.image_url}`} size="sm" />
                <div className="bg-gray-100 py-2 px-3 rounded-2xl dark:bg-neutral-700">
                  <p>{message.body}</p>
                </div>
              </div>
            )
